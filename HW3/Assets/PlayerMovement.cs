    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.GetComponent<Mass>())
        {
            if (collision.gameObject.GetComponent<Mass>().mas < _rigidbody.mass)  
            {
                ...
                Vector3 direction = _rigidbody.position - collision.gameObject.GetComponent<Mass>().transform.position;
                collision.gameObject.GetComponent<Mass>().rd.AddForce(direction * (-5), ForceMode.Impulse);
            }
            else
            {
